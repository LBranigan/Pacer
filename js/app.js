...
const CODE_VERSION = 'v41-2026-02-21';
...
async function runAnalysis() {
  ...
  // Must run AFTER all forgiveness passes (Pk trust Phase 7b, post-struggle
  // leniency Phase 8, end-of-reading) so that entry.forgiven is finalized.
  // Four conditions:
  ...
  // to precede the correct production.
  {
    ...
    for (let i = 0; i < alignment.length; i++) {
      const entry = alignment[i];
      if (entry.type === 'insertion' || entry.type === 'omission') continue;
      if (entry._notAttempted) continue;
