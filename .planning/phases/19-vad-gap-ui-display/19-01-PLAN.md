...
must_haves:
  truths:
    - "Hovering over pause indicator shows tooltip with VAD percentage, acoustic label in parentheses, and factual hint"
    - "Hovering over hesitation indicator shows tooltip with VAD percentage, acoustic label in parentheses, and factual hint"
...
Purpose: Teachers currently see pause and hesitation indicators but cannot tell if the gap was true silence or contained sounding-out / timestamp drift. This plan adds tooltips showing VAD percentage with acoustic label and factual descriptions, plus visual distinction (orange color) for gaps with significant speech activity.
...
- Tooltip format: "VAD: X% (acoustic label) - factual hint" (shows both label and hint per user decision)
...
This function will be called by both pause and hesitation indicator rendering. The format "VAD: X% (label) - hint" honors the CONTEXT.md decision to show acoustic label in full form with parentheses while also including the factual description.
  </action>
  <verify>grep -n "buildVADTooltipInfo" js/ui.js shows the function definition</verify>
  <done>Helper function exists and produces format "VAD: X% (acoustic label) - factual hint" for all 5 acoustic labels</done>
</task>
...
```
  ...
  <done>Pause indicators show "VAD: X% (label) - hint" in tooltip; hesitation indicators show same; both add VAD class when >= 30%</done>
</task>
...
2. Verify pause indicators:
   - Hover shows tooltip with "Long pause: Xms (error: >= 3000ms)" AND "VAD: X% (label) - factual hint"
   - If VAD >= 30%, pause text should be orange
   - If VAD < 30%, pause text should be default gray (#999)

3. Verify hesitation indicators:
   - Hover shows tooltip with hesitation info AND "VAD: X% (label) - factual hint"
...
<success_criteria>
- [ ] UI-01: Hovering over pause indicator shows "VAD: X% (label) - factual hint" in tooltip
- [ ] UI-02: Hovering over hesitation indicator shows "VAD: X% (label) - factual hint" in tooltip
